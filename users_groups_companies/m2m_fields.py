    ...
    for act in values:
        ...
        if act[0] == 0:
            ...
            if rel == 'res_groups_users_rel' and company_id:
                cr.execute('insert into '+rel+' ('+id1+','+id2+', company_id, active) values (%s,%s,%s, True)', (id, idnew, company_id))
            else:
            ...
            if rel == 'res_groups_users_rel' and company_id:
                ...
                if not cr.fetchone():
                    cr.execute('insert into '+rel+' ('+id1+','+id2+', company_id, active) values (%s,%s,%s,True)', (id, act[1], company_id))
            else:
            ...
            for act_nbr in act[2]:
                ...
                if rel == 'res_groups_users_rel' and company_id:
                    cr.execute('insert into '+rel+' ('+id1+','+id2+', company_id, active) values (%s, %s, %s, True)', (id, act_nbr, company_id))
                else:
...
def _get_query_and_where_params(self, cr, model, ids, values, where_params):
    """ Extracted from ``get`` to facilitate fine-tuning of the generated
        query. """
    if values['rel'] == 'res_groups_users_rel':
        query = 'SELECT %(rel)s.%(id2)s, %(rel)s.%(id1)s \
                   FROM %(rel)s, %(from_c)s \
                  WHERE %(rel)s.%(id1)s IN %%s \
                    AND %(rel)s.%(id2)s = %(tbl)s.id \
                    AND res_groups_users_rel.active = True \
                 %(where_c)s  \
                 %(order_by)s \
                 %(limit)s \
                 OFFSET %(offset)d' \
...
def patch():
    ...
    openerp.osv.fields.many2many._get_query_and_where_params = _get_query_and_where_params
